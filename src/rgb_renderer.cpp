...
//            img_ptr->encoding = sensor_msgs::image_encodings::TYPE_8UC4;
//            img_ptr->data.resize(img_ptr->height*img_ptr->width*sizeof(float));
//            img_ptr->step = img_ptr->width*sizeof(float);
            img_ptr->encoding = sensor_msgs::image_encodings::RGB8;
            img_ptr->data.resize(img_ptr->height*img_ptr->width*3);
            img_ptr->step = img_ptr->width*3;
            img_ptr->is_bigendian = false;
            img_ptr->header.frame_id ="/graspit_camera";
            img_ptr->header.stamp = ros::Time::now();

            //std::cout << "rgb Q image height: " << image.size().height() << " width: " << image.size().width() << std::endl;
            //std::cout << "rgb image height: " << img_ptr->height << " width: " << img_ptr->width  << std::endl;

              int channels = 3;
              unsigned char * sensor_data = (unsigned char *)img_ptr->data.data();

              for(int y = 0; y < img_ptr->height; ++y, sensor_data+=img_ptr->width*3)
              {
